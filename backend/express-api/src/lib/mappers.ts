...
/**
 * Lean job mapper for list/feed endpoints — omits heavy text fields.
 * Reduces feed payload by ~60% vs full mapJob().
 * Use for: listJobs, search results, recommendation feeds.
 */
export function mapJobSummary(raw: any): JobDTO {
  return {
    id: raw.id,
    title: raw.title,
    slug: raw.slug,
    companyId: raw.companyId,
    company: raw.company ? mapCompanySummary(raw.company) : ({} as CompanySummaryDTO),
    // Omit description, requirements, benefits — too heavy for list views
    description: '',
    requirements: [],
    benefits: [],
    skills: raw.skills ?? [],
