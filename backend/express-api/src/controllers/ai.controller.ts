...
export class AIController {
  ...
  async searchHybrid(req: Request, res: Response, next: NextFunction) {
    try {
      const { q } = req.body
      if (!q) {
        throw new AppError('Search query q is required in body', 400)
      }

      const jobsResult = await jobRepository.findAll({ limit: 50, filters: {} })
      const jobs = jobsResult.data

      if (jobs.length === 0) {
        return sendSuccess(res, [], 'No jobs available for hybrid search')
      }

      const scores = await aiService.getSemanticSearch(q, jobs)
      
      const ranked = scores.map((s) => {
        const job = jobs.find((j) => j.id === s.jobId)
        if (!job) return null
        const keywordScore = job.title.toLowerCase().includes(q.toLowerCase()) ? 100 : 50
        const overallScore = Math.round((0.4 * keywordScore) + (0.6 * s.score))
        return {
          ...job,
          aiMatchScore: overallScore,
        }
      }).filter((j): j is any => j !== null).sort((a, b) => (b.aiMatchScore || 0) - (a.aiMatchScore || 0))


      return sendSuccess(res, ranked, 'Hybrid search ranked results retrieved')
    } catch (error) {
      next(error)
    }
  }
  ...
  async chatAssistant(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!message) {
        throw new AppError('Message is required in body', 400)
      }

      await aiService.streamChatAssistant(message, context || {}, res)
    } catch (error) {
      next(error)
    }
  }
